...
| MCP             | `@modelcontextprotocol/server` (stdio transport)                |
...
| 5   | JD extraction & track       | `jho track <url>` creates app with JD + suggested role; `jho track <slug>` updates; `jho list --role` filters; `jho stats` prints snapshot | `feat(jobs+applications): URL fetch, JD extract, application CRUD, list, stats` |
| 6   | Cover letter & Q&A          | `jho cover-letter`, `jho answer` work                                                     | `feat(generation): cover letter and application Q&A`                                 |
| 7   | Applications depth               | interviews, doctor, repair, ownership, show                                               | `feat(applications): interviews, doctor, repair, ownership table`                         |
