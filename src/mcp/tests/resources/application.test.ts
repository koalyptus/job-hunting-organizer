...
describe('application resource', () => {
  ...
  it('returns application data', async () => {
    const { client } = await createTestServer(registerApplication);
    const result = await client.readResource({
      uri: 'jho://applications/default/test-app',
    });
  ...
  it('returns error when campaign parameter is missing', async () => {
    const { readCallbacks } = await createTestServerAndCapture(registerApplication);
    const handler = readCallbacks[0]!;

    const result = await invokeResourceRead(handler, 'jho://applications/default/test-app', {
