...
export function registerCoverLetter(server: McpServer): void {
  server.registerTool('cover_letter', { description: 'Generate a tailored cover letter for an application', inputSchema: CoverLetterInput }, async (args) => {
              try {
...
export function registerReadCoverLetter(server: McpServer): void {
  server.registerTool('read_cover_letter', { description: 'Read an existing saved cover letter for an application', inputSchema: ReadCoverLetterInput }, async (args) => {
              try {
                mcpLogger.debug(
