...
export function registerAggregateRetros(server: McpServer): void {
  server.registerTool('aggregate_retros', { description: 'Aggregate weak topics across all application retro files for a campaign', inputSchema: AggregateRetrosInput }, async (args) => {
              try {
