...
export function registerShowApplication(server: McpServer): void {
  server.registerTool('show_application', { description: 'Show a single application: metadata (meta.md) and job description (jd.md)', inputSchema: ShowApplicationInput }, async (args) => {
              try {
                mcpLogger.debug(
