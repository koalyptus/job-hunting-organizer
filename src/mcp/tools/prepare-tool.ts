...
export function registerPrepare(server: McpServer): void {
  server.registerTool('prepare', { description: 'Generate or add topics to a pre-interview prep plan. Provide topics to append to an existing plan (steer/days ignored in this mode).', inputSchema: PrepareInput }, async (args) => {
              try {
