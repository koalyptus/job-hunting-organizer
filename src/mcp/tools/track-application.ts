...
export function registerTrackApplication(server: McpServer): void {
  server.registerTool('track_application', { description: 'Track a job application — create from URL or update by slug', inputSchema: TrackApplicationInput }, async (args) => {
              try {
                mcpLogger.debug(
