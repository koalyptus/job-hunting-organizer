...
describe('MetaFrontmatterSchema', () => {
  ...
  it('rejects invalid status', () => {
    ...
    expect(() => MetaFrontmatterSchema.parse(raw)).toThrow();
  });
...
describe('validateMetaFrontmatter', () => {
  it('returns validated data on success', () => {
    const raw = {
      slug: '2026-Jun-03-SE-Foo',
      appliedOn: '2026-06-03',
    };
    const result = validateMetaFrontmatter(raw);
  ...
  it('throws on invalid status', () => {
    ...
    expect(() => validateMetaFrontmatter(raw)).toThrow();
  });
});

describe('safeValidateMetaFrontmatter', () => {
  it('returns success: true with data on valid input', () => {
    const raw = {
      slug: '2026-Jun-03-SE-Foo',
      appliedOn: '2026-06-03',
    };
    const result = safeValidateMetaFrontmatter(raw);
